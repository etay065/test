...
class MEC_feature_ix extends MEC_base
{
    ...
    public function import_start_bookings()
    {
        ...
        if(!isset($feed_file['name']) or (isset($feed_file['name']) and trim($feed_file['name']) == '')) return array('success' => 0, 'message' => __('Please upload a CSV file.', 'mec'));

        // File Type is not valid
        if(!isset($feed_file['type']) or (isset($feed_file['type']) and !in_array(strtolower($feed_file['type']), array('text/csv', 'application/vnd.ms-excel')))) return array('success' => 0, 'message' => __('The file type should be CSV.', 'mec'));
        ...
        if(($h = fopen($target_path, 'r')) !== false)
        {
            ...
            while(($data = fgetcsv($h, 1000, ",")) !== false)
            {
                ...
                if($confirmed_label == __('Confirmed', 'mec')) $confirmed = 1;
                elseif($confirmed_label == __('Rejected', 'mec')) $confirmed = -1;
                else $confirmed = 0;

                $verified_label = $data[12];
                if($verified_label == __('Verified', 'mec')) $verified = 1;
                elseif($verified_label == __('Canceled', 'mec')) $verified = -1;
                else $verified = 0;
        ...
        return array('success' => 1, 'message' => __('The bookings are imported successfully!', 'mec'));
    }
    ...
    public function g_calendar_import_start()
    {
        ...
        if(!trim($api_key) or !trim($calendar_id)) return array('success'=>0, 'error'=>__('API key and Calendar ID are required!', 'mec'));
    ...
    public function g_calendar_import_do()
    {
        ...
        if(!count($g_events)) return array('success'=>0, 'error'=>__('Please select events to import!', 'mec'));
    ...
    public function meetup_import_start()
    {
        ...
        if(!trim($api_key) or !trim($group_url)) return array('success'=>0, 'error'=>__('API key and Group URL are required!', 'mec'));
    ...
    public function meetup_import_do()
    {
        ...
        if(!count($m_events)) return array('success'=>0, 'error'=>__('Please select events to import!', 'mec'));
    ...
    public function g_calendar_export_authenticate()
    {
        ...
        $this->main->response(array('success'=>1, 'message'=>sprintf(__('All seems good! Please click %s to authenticate your app.', 'mec'), '<a href="'.$auth_url.'">'.__('here', 'mec').'</a>')));
    }
    ...
    public function g_calendar_export_do()
    {
        ...
        if(!trim($client_id) or !trim($client_secret) or !trim($calendar_id)) $this->main->response(array('success'=>0, 'message'=>__('Client App, Client Secret, and Calendar ID are all required!', 'mec')));
        ...
        $message .= (count($g_events_updated) ? ' '.sprintf(__('%s Updated previously added events.', 'mec'), '<strong>'.count($g_events_updated).'</strong>') : '');
    ...
    public function f_calendar_import_start()
    {
        ...
        if(!$fb_page_id)
        {
            $message = __("We were not able to recognize your Facebook page. Please check again and provide a valid link.", 'mec');
    ...
    public function f_calendar_import_do()
    {
        ...
        if(!count($f_events)) return array('success'=>0, 'message'=>__('Please select events to import!', 'mec'));
        ...
        $fb_page_id = isset($fb_page['id']) ? $fb_page['id'] : 0;
        if(!$fb_page_id) return array('success'=>0, 'message'=>__("We were not able to recognize your Facebook page. Please check again and provide a valid link.", 'mec'));
