...
class MEC_feature_mec extends MEC_base
{
    ...
    public function import_settings()
    {
        ...
        if($options == 'No-JSON')
        {
            echo '<div class="mec-message-import-error">' . esc_html__('Your option is not in JSON format. Please insert correct options in this field and try again.' , 'mec') . '</div>';
            exit();
        }
        else
        {
    ...
    public function menus()
    {
        ...
        {
            add_submenu_page('mec-intro', __('MEC - Report', 'mec'), __('Report', 'mec'), 'manage_options', 'MEC-report', array($this, 'report'));
        }
        if (!$this->getPRO()) add_submenu_page('mec-intro', __('MEC - Go Pro', 'mec'), __('Go Pro', 'mec'), 'manage_options', 'MEC-go-pro', array($this, 'go_pro'));
        ...
        return '<div class="mec-form-row mec-sed-method-wrap">
            ...
            <div class="mec-col-4">
                ...
                <ul class="mec-sed-methods" data-for="#mec_skin_'.$skin.'_sed_method_field">
                    ...
                    <li data-method="m1" class="'.($value === 'm1' ? 'active' : '').'">'.__('Modal Popup', 'mec').'</li>
                </ul>
            </div>
    ...
    public function booking_button_field($skin, $value = 0)
    {
        ...
        return '<div class="mec-form-row mec-booking-button-wrap mec-switcher">
            <div class="mec-col-4">
                <label for="mec_skin_'.$skin.'_booking_button">'.__('Booking Button / Icon', 'mec').'</label>
            </div>
            <div class="mec-col-4">
    ...
    public function widget_news()
    {
        ...
        echo '<div class="mec-metabox-head-wrap">
            <div class="mec-metabox-head-version">
                ...
                <p>'.($this->getPRO() ? __('Modern Events Calendar', 'mec') : __('Modern Events Calendar (Lite)', 'mec')).'</p>
        ...
        echo '<div class="mec-metabox-upcoming-wrap"><h3 class="mec-metabox-feed-head">'.esc_html__('Upcoming Events' , 'mec').'</h3><ul>';
        ...
        if(!empty($obj))
        {
            echo '<h3 class="mec-metabox-feed-head">'.esc_html__('News & Updates' , 'mec').'</h3><div class="mec-metabox-feed-content"><ul>';
        ...
        // Links
        echo '<div class="mec-metabox-footer"><a href="https://webnus.net/blog/" target="_blank">'.esc_html__('Blog', 'mec').'<span aria-hidden="true" class="dashicons dashicons-external"></span></a><a href="https://webnus.net/dox/modern-events-calendar/" target="_blank">'.esc_html__('Help', 'mec').'<span aria-hidden="true" class="dashicons dashicons-external"></span></a>';
        if($this->getPRO()) echo '<a href="https://webnus.net/mec-purchase" target="_blank">'.esc_html__('Go Pro', 'mec').'<span aria-hidden="true" class="dashicons dashicons-external"></span></a>';
        echo '</div>';
    }
                    ...
                    <div class="w-box-content">
                        ...
                        <ul>
                            <li><a href="<?php echo add_query_arg(array(
                                ...
                                'type' => 'daily',
                                'chart' => $chart,
                            )); ?>"><?php _e('This Month', 'mec'); ?></a></li>
                            <li><a href="<?php echo add_query_arg(array(
                                'start' => date('Y-m-01', strtotime('-1 Month')),
                                'end' => date('Y-m-t', strtotime('-1 Month')),
                                'type' => 'daily',
                                'chart' => $chart,
                            )); ?>"><?php _e('Last Month', 'mec'); ?></a></li>
                            <li><a href="<?php echo add_query_arg(array(
                                'start' => date('Y-01-01'),
                                'end' => date('Y-12-31'),
                                'type' => 'monthly',
                                'chart' => $chart,
                            )); ?>"><?php _e('This Year', 'mec'); ?></a></li>
                            <li><a href="<?php echo add_query_arg(array(
                                'start' => date('Y-01-01', strtotime('-1 Year')),
                                'end' => date('Y-12-31', strtotime('-1 Year')),
                                'type' => 'monthly',
                                'chart' => $chart,
                            )); ?>"><?php _e('Last Year', 'mec'); ?></a></li>
                        </ul>
                ...
                <div class="w-box total-bookings print-events">
                    ...
                    <div class="w-box-content">
                        <form method="GET" action="<?php echo home_url(); ?>" target="_blank">
                            ...
                            <select name="mec-month" title="<?php esc_attr('Month', 'mec'); ?>">
                                <?php for($i = 1; $i <= 12; $i++): ?>
                            ...
                            <button type="submit"><?php _e('Display Events', 'mec'); ?></button>
                        </form>
                    </div>
                </div>
            </div>
