...
class MEC_feature_books extends MEC_base
{
    ...
    ?>
        <div class="mec-book-details">
            ...
            <div class="mec-attendee">
                ...
                ?>
                <?php if(isset($reg_form) && !empty($reg_form)): foreach($reg_form as $field_id=>$value): $label = isset($reg_fields[$field_id]) ? $reg_fields[$field_id]['label'] : ''; $type = isset($reg_fields[$field_id]) ? $reg_fields[$field_id]['type'] : ''; ?>
                            ...
                            <select id="mec_book_form_date" class="widefat mec-booking-edit-form-dates" name="mec_date">
                                ...
                                <?php foreach($occurrences as $occurrence): $occ_timestamp = $this->book->timestamp($occurrence['start'], $occurrence['end']); ?>
                                    <option value="<?php echo $occ_timestamp; ?>" <?php echo (($meta['mec_date'] == $occ_timestamp or $meta['mec_date'] == $occurrence['start']['date'].':'.$occurrence['end']['date']) ? 'selected="selected"' : ''); ?>>
                                        <?php echo strip_tags($this->main->date_label($occurrence['start'], $occurrence['end'], $date_format)); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
    ...
    public function save_book($post_id)
    {
        ...
        if($is_new_booking)
        {
            ...
            $post_id = $this->book->add(array(
                ...
                'post_author'=>$user_id,
                'post_type'=>$this->PT,
                'post_title'=>$name.' - '.get_userdata($user_id)->user_email,
                'post_date'=>$date
    ...
    public function bbf_date_tickets_booking_form()
    {
        ...
        // Date Option
        $date_options = '';
        foreach($dates as $date) $date_options .= '<option value="'.$this->book->timestamp($date['start'], $date['end']).'">'.strip_tags($this->main->date_label($date['start'], $date['end'], $date_format)).'</option>';
