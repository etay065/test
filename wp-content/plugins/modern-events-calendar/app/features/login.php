...
class MEC_feature_login extends MEC_base
{
    ...
    public function mec_ajax_login_data()
    {
        ...
        {
            echo json_encode(array('loggedin'=>false, 'message'=>__('<strong>'.esc_html__('Wrong username or password, reloading...' , 'mec').'</strong>')));
        }
        else
        {
            echo json_encode(array('loggedin'=>true, 'message'=>__('<strong>'.esc_html__('Login successful, redirecting...' , 'mec').'</strong>')));
        }
