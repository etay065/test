...
class MEC_feature_search extends MEC_base
{
    ...
    public function show_taxonomy($taxonomy, $icon)
    {
        ...
        switch($taxonomy_name)
        {
            // Message Category
            case 'category':
                $taxonomy_name = $this->main->m('taxonomy_category', __('Category', 'mec'));
                $taxonomy_key = 'category';
                break;

            // Message Location
            case 'location':
                $taxonomy_name = $this->main->m('taxonomy_location', __('Location', 'mec'));
                $taxonomy_key = 'location';
                break;

            // Message Organizer
            case 'organizer':
                $taxonomy_name = $this->main->m('taxonomy_organizer', __('Organizer', 'mec'));
                $taxonomy_key = 'organizer';
                break;

            // Message Organizer
            case 'speaker':
                $taxonomy_name = $this->main->m('taxonomy_speaker', __('Speaker', 'mec'));
                $taxonomy_key = 'speaker';
                break;

            // Message Tag
            case 'tag':
                $taxonomy_name =  __('Tag', 'mec');
                $taxonomy_key = 'tag';
                break;

            // Message label
            case 'label':
                $taxonomy_name = $this->main->m('taxonomy_label', __('Label', 'mec'));
                $taxonomy_key = 'label';
                break;
    ...
    public function mec_get_ajax_search_data()
    {
        if($_POST['length'] < '3')
        {
            _e('Please enter at least 3 characters and try again' , 'mec');
            die();
        }
