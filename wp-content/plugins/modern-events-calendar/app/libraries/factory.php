...
class MEC_factory extends MEC_base
{
    ...
    {
        add_menu_page(__('M.E. Calendar', 'mec'), __('M.E. Calendar', 'mec'), 'edit_posts', 'mec-intro', array($this->main, 'dashboard'), plugin_dir_url(__FILE__ ) . '../../assets/img/mec.svg', 26);
    }
    ...
    public function load_plugin_links($links, $file)
    {
        if(strpos($file, MEC_DIRNAME) !== false)
        {
            if(!$this->getPRO())
            {
                ...
                $rate    =  '<a href="https://wordpress.org/support/plugin/modern-events-calendar-lite/reviews/#new-post" target="_blank">'._x('Rate the plugin ★★★★★', 'plugin rate', 'mec').'</a>';
                $links[] = $upgrade;
                $links[] = $rate;
            }
        }
    ...
    public function mec_body_class($dark)
    {
        ...
        if( !empty($dark_mode) xor $dark_mode == 1) $dark[] = 'mec-dark-mode';
