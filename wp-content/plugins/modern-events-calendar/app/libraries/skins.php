...
class MEC_skins extends MEC_base
{
    ...
    public function booking_button($event, $type = 'button') 
    {
        ...
        if($type === 'button') return '<a class="mec-modal-booking-button mec-mb-button" href="'.esc_url($link).'" '.$modal.'>'.$this->main->m('register_button', __('Book Event', 'mec')).'</a>';
        else return '<a class="mec-modal-booking-button mec-mb-icon" title="' . __('Book Event', 'mec') . '" href="'.esc_url($link).'" '.$modal.'><i class="mec-sl-note"></i></a>';
    }
}