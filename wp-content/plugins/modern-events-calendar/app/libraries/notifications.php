...
class MEC_notifications extends MEC_base
{
    ...
    public function email_verification($book_id)
    {
        ...
        $subject = isset($this->notif_settings['email_verification']['subject']) ? __($this->notif_settings['email_verification']['subject'], 'mec') : __('Please verify your email.', 'mec');
    ...
    public function booking_notification($book_id)
    {
        ...
        $subject = isset($this->notif_settings['booking_notification']['subject']) ? __($this->notif_settings['booking_notification']['subject'], 'mec') : __('Your booking is received.', 'mec');
    ...
    public function booking_confirmation($book_id, $mode = 'manually')
    {
        ...
        $subject = isset($this->notif_settings['booking_confirmation']['subject']) ? __($this->notif_settings['booking_confirmation']['subject'], 'mec') : __('Your booking is confirmed.', 'mec');
    ...
    public function booking_cancellation($book_id)
    {
        ...
        $subject = isset($this->notif_settings['cancellation_notification']['subject']) ? __($this->notif_settings['cancellation_notification']['subject'], 'mec') : __('booking canceled.', 'mec');
    ...
    public function admin_notification($book_id)
    {
        ...
        $subject = isset($this->notif_settings['admin_notification']['subject']) ? __($this->notif_settings['admin_notification']['subject'], 'mec') : __('A new booking is received.', 'mec');
    ...
    public function booking_reminder($book_id)
    {
        ...
        $subject = isset($this->notif_settings['booking_reminder']['subject']) ? __($this->notif_settings['booking_reminder']['subject'], 'mec') : __('Booking Reminder', 'mec');
    ...
    */
    public function user_event_publishing($new, $old, $post)
    {
        ...
        if(($new == 'publish') and ($old != 'publish') and ($post->post_type == $event_PT))
        {
            ...
            $to = $email;
            $subject = (isset($this->notif_settings['user_event_publishing']['subject']) and trim($this->notif_settings['user_event_publishing']['subject'])) ? __($this->notif_settings['user_event_publishing']['subject'], 'mec') : __('Your event is published.', 'mec');
    ...
    public function content($message, $book_id, $attendee = array())
    {
        ...
        if(trim($timestamps) and strpos($timestamps, ':') !== false)
        {
            ...
            {
                $book_date = sprintf(__('%s to %s', 'mec'), $this->main->date_i18n($date_format, $start_timestamp), $this->main->date_i18n($date_format, $end_timestamp));
            }
        ...
        $event_time = $allday ? $this->main->m('all_day', __('All Day' , 'mec')) : (!$hide_end_time ? sprintf(__('%s to %s', 'mec'), $event_start_time, $event_end_time) : $event_start_time);
        ...
        if(trim($timestamps) and strpos($timestamps, ':') !== false)
        {
            if(trim($start_timestamp) != trim($end_timestamp))
            {
                $book_datetime = sprintf(__('%s to %s', 'mec'), $this->main->date_i18n($date_format.((!$allday and !$hide_time) ? ' '.$time_format : ''), $start_timestamp), $this->main->date_i18n($date_format.((!$allday and !$hide_time and !$hide_end_time) ? ' '.$time_format : ''), $end_timestamp));
            }
        ...
        foreach($next_occurrences as $next_occurrence)
        {
            ...
            if(isset($next_occurrence['tstart']) and trim($next_occurrence['tstart']) and isset($next_occurrence['tend']) and trim($next_occurrence['tend']))
            {
                ...
                {
                    $book_date_next_occurrences .= sprintf(__('%s to %s', 'mec'), $this->main->date_i18n($date_format, $next_occurrence['tstart']), $this->main->date_i18n($date_format, $next_occurrence['tend'])).'<br>';
                    $book_datetime_next_occurrences .= sprintf(__('%s to %s', 'mec'), $this->main->date_i18n($date_format.((!$allday and !$hide_time) ? ' '.$time_format : ''), $next_occurrence['tstart']), $this->main->date_i18n($date_format.((!$allday and !$hide_time and !$hide_end_time) ? ' '.$time_format : ''), $next_occurrence['tend'])).'<br>';
                }
                else
                {
    ...
    public function get_full_attendees_info($book_id)
    {
        ...
        if(count($attachments))
        {
            $attendees_full_info .= __($attachment_field['label'], 'mec').': <a href="'.esc_url($attachments[0]['url']).'" target="_blank">'.esc_url($attachments[0]['url']).'</a>'."\r\n";
        }
